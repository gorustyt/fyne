	...
	expected := []mockRunner{
		{
			...
			mockReturn: mockReturn{
				ret: []byte("{ \"Module\": { \"Path\": \"github.com/gorustyt/fyne/v2\"} }"),
			},
		},
		{
			expectedValue: expectedValue{args: []string{"version"}},
...
func Test_FyneGoMod(t *testing.T) {
	jsonTests := []jsonTest{
		{false, []byte(`{"Module": {"Path": "github.com/fyne-io/calculator"},"Go": "1.14",	"Require": [ { "Path": "github.com/gorustyt/fyne/v2","Version": "v2.1.4"} ] }`)},
		{true, []byte(`{ "Module": {"Path": "github.com/gorustyt/fyne/v2"},"Require": [{ "Path": "test","Version": "v2.1.4"} ] }`)},
		{true, []byte(`{"Module": {"Path": "github.com/fyne-io/calculator"},"Go": "1.14",	"Require": [ { "Path": "github.com/gorustyt/fyne/v2","Version": "v2.2.0"} ] }`)},
	}
